...
The skill ships with its own regression suite in [`evals/`](evals/): thirteen
scenarios over twelve disposable fixtures, each guarding a specific failure mode of "the work
...
during a plan-only dry run, vacuous new tests that survive tampering, and
fixes whose covering test already passed pre-fix). Every run is benchmarked **With Skill**
