...
2. **Tier 1 — static**: typecheck, lint, build, using the project's own
   scripts/Makefile/CI definitions. Absent = SKIP with reason.
3. **Tier 2 — tests**: run the relevant suites. For a bug fix, prove the
...
4. **Tier 3 — runtime**: run the real thing and observe the changed flow.
...
5. **Verdicts**: PASS / FAIL / BLOCKED / SKIP per tier, overall verdict
