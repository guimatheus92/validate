...
- Causal proof and deployed incidence are independent claims. A pre-fix
  FAIL / post-fix PASS proves the fix works where you ran it — it says
  nothing about whether the changed path is deployed, executed, or failing
  in any real system. Local PASS never implies deployed truth; when the
  deployed-evidence gate applies, report both, separately
...
## Step 7 — deployed evidence (when applicable)

The gate declared in Step 3 decides this phase
([reference/deployed-evidence.md](reference/deployed-evidence.md)). FALSE →
continue to Step 8, with the labeled reason in the report. TRUE → follow
the reference end to end: source discovery, then the ask — provide
coordinates / name another source / waive — happens here, never in Step 3;
an explicit decision already in the user's request consumes the ask.
UNKNOWN → ask; proceeding without an answer makes the phase BLOCKED with
